  ...
  if (gameNode.value != null) {
    ...
    if (snapshot.value != null) {
      ...
      return {
        ...
        'gameID': decryptedID,
        'nodeExist': true,
        'count': int.parse(snapshot.value['details']['count']),
      };
    } else
...
class TissueBoxScreen extends StatefulWidget {
...
class ShadowScreenState extends State<TissueBoxScreen> {
  bool capPop;
  ...
  void initState() {
    ...
    timerFinished = false;
    seconds = '';
    globals.TissueGlobalKey = _keyChild1;
    gameLoadedorNot();
  }
...
class TissueBoxView extends StatefulWidget {
...
class TissueBoxViewState extends State<TissueBoxView>
  ...
  void initState() {
    ...
    if (!widget.data['nodeExist'] && !widget.data['enterBefore']) {
      ...
      for (int i = 0; i < widget.data['count']; i++) {
        tissues.add(
...
///////////////////////////////////////////////////////////////CHECK FIREBASE FOR NEW GAME ON WIDGET RESUME
  ...
  Future gameComplete() async {
    ...
    try {
      await Future.wait([TissueBoxScreen.of(context).takeScreenShot()])
          .then((value) async {
        setState(() {
          isLoading = true;
        });
  ...
  Future incrementCount() {
    if (tissueCount < widget.data['count']) {
      setState(() {
        tissueCount++;
      });
      if (tissueCount == widget.data['count']) gameComplete();
    } else
      gameComplete();
  }
