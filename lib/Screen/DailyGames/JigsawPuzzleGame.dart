...
import 'package:win_diggy/Models/JigsawPuzzleModels/GamePainter.dart';
...
Future<Map<String, dynamic>> fetchCountry(http.Client client) async {
  ...
  if (gameNode.value != null) {
    ...
    if (snapshot.value != null) {
      for (var pos in snapshot.value['Details']['Start']) {
        posList.add(int.parse(pos));
      }
      await DefaultCacheManager()
          .getFile(snapshot.value['Details']['ImageLink']);
      ...
      Uint8List image;
      image = await _networkImageToByte(snapshot.value['Details']['ImageLink']);
      _imageLoader = await loadImage(image);

      return {
        'gameID': decryptedID,
        'image': snapshot.value['Details']['ImageLink'],
        'byteImage': _imageLoader,
        'position': posList,
        'size': int.parse(snapshot.value['Details']['size']),
        'enterBefore': false,
        'nodeExist': true
      };
    } else
...
class JigsawPuzzleScreenState extends State<JigsawPuzzleScreen> {
  bool capPop;
  ScreenshotController screenshotController = ScreenshotController();
  GlobalKey<JigsawViewState> _keyChild1 = GlobalKey();
  ...
  void initState() {
    ...
    timerFinished = false;
    seconds = '';
    globals.JigsawGlobalKey = _keyChild1;
    gameLoadedorNot();
  }
...
class JigsawView extends StatefulWidget {
  ...
  List<int> pos;
  GlobalKey key;
  JigsawView(this.size, this.image, this.level, this.data, this.pos, this.key);
  ...
  JigsawViewState(this.size, this.byteImage, this.level, this.posList) {
    ...
    puzzleMagic.init(byteImage, size, level).then((val) async {
      setState(() {
        ...
        nodes = GameEngine.makeRandom(nodes, posList);
        setState(() {
...
///////////////////////////////////////////////////////////////CHECK FIREBASE FOR NEW GAME ON WIDGET RESUME
  ...
  Future gameComplete() async {
    ...
    try {
      await Future.wait([JigsawPuzzleScreen.of(context).takeScreenShot()])
          .then((value) async {
        setState(() {
          isLoading = true;
        });
  ...
  void showStartAnimation() {
    ...
    nodes.forEach((node) {
      ...
      print(' CURRENT INDEX: ${node.curIndex}');
