...
Future<Map<String, dynamic>> fetchCountry(http.Client client) async {
  ...
  if (snapshot.value != null) {
    DataSnapshot gameData = await FirebaseDatabase.instance
        .reference()
        .child('game-' + decryptedID)
        .child('game-data')
        .once();
    if (gameData.value != null) {
      ...
      return {
        'colors': gameData.value['details']['number_of_colors'],
        'taps': gameData.value['details']['number_of_tabs'],
        'enterBefore': false,
        'gameID': decryptedID,
        'nodeExist': true
      };
...
class TapTapScreen extends StatefulWidget {
...
class TapTapScreenState extends State<TapTapScreen> {
  ...
  void initState() {
    ...
    timerFinished = false;
    seconds = '';
    globals.TapGlobalKey = _keyChild1;
    gameLoadedorNot();
  }
...
class TapTapInnerViewState extends State<TapTapInnerView>
  ...
  void initState() {
    ...
    if (!widget.mapData['nodeExist'] && !widget.mapData['enterBefore']) {
      setState(() {
        corruptGame = true;
      });
    } else if (widget.mapData['nodeExist'] && widget.mapData['enterBefore']) {
      setState(() {
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) async {
    ...
    if (state == AppLifecycleState.resumed) {
      updateListener = FirebaseDatabase.instance
          .reference()
          .child('game-' + widget.mapData['gameID'])
          .onChildChanged
  ...
  Future setcolors() async {
    print('NO OF COLORS: $totalColors');
    if (totalColors == 5) {
      print(totalColors);
      setState(() {
  ...
  Future singleButtonTap() async {
    ...
    if (contHeight.round() >= barHeight2.round()) {
      ...
      try {
        await Future.wait([TapTapScreen.of(context).takeScreenShot()])
            .then((value) async {
          setState(() {
            isLoading = true;
          });
