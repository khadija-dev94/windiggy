...
import 'package:win_diggy/Widgets/MSQCard.dart';
...
Future<Map<String, dynamic>> fetchCountry(http.Client client) async {
  ...
  if (gameNode.value != null) {
    ...
    if (snapshot.value != null) {
      ...
      for (var data in snapshot.value['game']) {
        ...
        if (data['correct_ans'] == 'simple') {
          ...
          if (data['mcq_type'] == 'image') {
            ...
            if (!gameLoaded) {
              await DefaultCacheManager().getFile(data['mcq_images']);
            }
          } else {
            questions.add(Question(
                data['question'],
...
class QuizScreen extends StatefulWidget {
  static QuizScreenState of(BuildContext context) =>
      context.ancestorStateOfType(const TypeMatcher<QuizScreenState>());
  @override
...
class QuizScreenState extends State<QuizScreen> {
  ...
  void initState() {
    ...
    timerFinished = false;
    seconds = '';
    globals.MCQGlobalKey = _keyChild1;
    gameLoadedorNot();
  }
...
class MSQSViewState extends State<MSQSView>
  ...
  void initState() {
    ...
    if (!widget.data['nodeExist'] && !widget.data['enterBefore']) {
      ...
      questionsList = widget.data['questions'];
      currentQue = 1;
  ...
  Future getData() async {
    ...
    Future.wait([checkFBOnWidgetResume()]).then((value) {
      setState(() {
        isLoading = false;
      });
    }).catchError((error) {
  ...
  Future checkNextState(int position, bool value) async {
    ...
    ///////////////////////////////////////////////////////////ALL MSQs DONE, CALL SERVER
      ...
      try {
        await Future.wait([QuizScreen.of(context).takeScreenShot()])
            .then((value) async {
          setState(() {
            isLoading = true;
          });
