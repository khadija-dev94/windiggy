...
Future<Map<String, dynamic>> fetchCountry(http.Client client) async {
  ...
  if (gameNode.value != null) {
    ...
    if (snapshot.value != null) {
      if (!gameLoaded) {
        for (var data in snapshot.value['stages']) {
          levels.add(Level(
...
class ShadowScreenState extends State<ShadowScreen> {
  bool capPop;
  GlobalKey<ShadowViewState> _keyChild1 = GlobalKey();
  ...
  void initState() {
    ...
    timerFinished = false;
    seconds = '';
    globals.ShadowGlobalKey = _keyChild1;
    gameLoadedorNot();
  }
...
class ShadowView extends StatefulWidget {
  ...
  ShadowView({this.data, this.key, this.gameID});
...
///////////////////////////////////////////////////////////////CHECK FIREBASE FOR NEW GAME ON WIDGET RESUME
  ...
  Future gameWon() async {
    ...
    await Future.delayed(Duration(seconds: 1));
    try {
      await Future.wait([ShadowScreen.of(context).takeScreenShot()])
          .then((value) async {
        setState(() {
          isLoading = true;
        });
